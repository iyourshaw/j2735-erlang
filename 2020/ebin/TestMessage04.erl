...
%% Purpose: Encoding and decoding of the types in TestMessage04.

-module('TestMessage04').
-moduledoc false.
-compile(nowarn_unused_vars).
-dialyzer(no_improper_lists).
-dialyzer(no_match).
-include("TestMessage04.hrl").
-asn1_info([{vsn,'5.3.1'},
            {module,'TestMessage04'},
...
try encode_jer('TestMessage04', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('TestMessage04', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
typeinfo('TestMessage04') ->
  {sequence,'TestMessage04',2,[{<<104,101,97,100,101,114>>,{typeinfo,{'Common','Header'}},'OPTIONAL'},{<<114,101,103,105,111,110,97,108>>,{sequence,'TestMessage04_regional',2,[{<<114,101,103,105,111,110,73,100>>,{'INTEGER',{0,255}},mandatory},{<<114,101,103,69,120,116,86,97,108,117,101>>,{'ObjClassFieldType',{regionId,1},{componentrelation,{objectset,121,{'Externaltypereference',undefined,'TestMessage04',internal_object_set_argument_1}},[{outermost,[{'Externalvaluereference',121,'Common',regionId}]}]}},mandatory}]},'OPTIONAL'}]};
typeinfo(Type) ->
