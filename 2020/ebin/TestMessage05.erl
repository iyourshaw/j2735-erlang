...
%% Purpose: Encoding and decoding of the types in TestMessage05.

-module('TestMessage05').
-moduledoc false.
-compile(nowarn_unused_vars).
-dialyzer(no_improper_lists).
-dialyzer(no_match).
-include("TestMessage05.hrl").
-asn1_info([{vsn,'5.3.1'},
            {module,'TestMessage05'},
...
try encode_jer('TestMessage05', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('TestMessage05', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
typeinfo('TestMessage05') ->
  {sequence,'TestMessage05',2,[{<<104,101,97,100,101,114>>,{typeinfo,{'Common','Header'}},'OPTIONAL'},{<<114,101,103,105,111,110,97,108>>,{sequence,'TestMessage05_regional',2,[{<<114,101,103,105,111,110,73,100>>,{'INTEGER',{0,255}},mandatory},{<<114,101,103,69,120,116,86,97,108,117,101>>,{'ObjClassFieldType',{regionId,1},{componentrelation,{objectset,121,{'Externaltypereference',undefined,'TestMessage05',internal_object_set_argument_1}},[{outermost,[{'Externalvaluereference',121,'Common',regionId}]}]}},mandatory}]},'OPTIONAL'}]};
typeinfo(Type) ->
