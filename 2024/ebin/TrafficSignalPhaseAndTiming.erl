...
%% Purpose: Encoding and decoding of the types in TrafficSignalPhaseAndTiming.
...
-export([typeinfo/1]).
-export([
enc_TrafficSignalPhaseAndTiming/1
]).

-export([
dec_TrafficSignalPhaseAndTiming/1
]).
...
try encode_jer('TrafficSignalPhaseAndTiming', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('TrafficSignalPhaseAndTiming', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
encode_disp('TrafficSignalPhaseAndTiming', Data) -> enc_TrafficSignalPhaseAndTiming(Data);
encode_disp(Type, _Data) -> exit({error,{asn1,{undefined_type,Type}}}).

decode_disp('TrafficSignalPhaseAndTiming', Data) -> dec_TrafficSignalPhaseAndTiming(Data);
...
enc_TrafficSignalPhaseAndTiming(Val) ->
[].


dec_TrafficSignalPhaseAndTiming(Bytes) ->
begin
ignore,
{'NULL',Bytes}
end.

typeinfo('TrafficSignalPhaseAndTiming') ->
  'NULL';
typeinfo(Type) ->
