...
%% Purpose: Encoding and decoding of the types in CommonSafetyRequest.
...
-dialyzer(no_match).
-include("CommonSafetyRequest.hrl").
-asn1_info([{vsn,'5.3.1'},
            {module,'CommonSafetyRequest'},
...
try encode_jer('CommonSafetyRequest', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('CommonSafetyRequest', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
typeinfo('CommonSafetyRequest') ->
  {sequence,'CommonSafetyRequest',5,[{<<116,105,109,101,83,116,97,109,112>>,{'INTEGER',{0,527040}},'OPTIONAL'},{<<109,115,103,67,110,116>>,{'INTEGER',{0,127}},'OPTIONAL'},{<<105,100>>,octet_string,'OPTIONAL'},{<<114,101,113,117,101,115,116,115>>,{typeinfo,{'CommonSafetyRequest','RequestedItemList'}},mandatory},{<<114,101,103,105,111,110,97,108>>,{sof,{sequence,'CommonSafetyRequest_regional_SEQOF',2,[{<<114,101,103,105,111,110,73,100>>,{'INTEGER',{0,255}},mandatory},{<<114,101,103,69,120,116,86,97,108,117,101>>,{'ObjClassFieldType',{regionId,1},{componentrelation,{objectset,134,{'Externaltypereference',undefined,'CommonSafetyRequest',internal_object_set_argument_1}},[{outermost,[{'Externalvaluereference',134,'Common',regionId}]}]}},mandatory}]}},'OPTIONAL'}]};
