...
%% Purpose: Encoding and decoding of the types in RoadGeometryAndAttributes.
...
-export([typeinfo/1]).
-export([
enc_RoadGeometryAndAttributes/1
]).

-export([
dec_RoadGeometryAndAttributes/1
]).
...
try encode_jer('RoadGeometryAndAttributes', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('RoadGeometryAndAttributes', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
encode_disp('RoadGeometryAndAttributes', Data) -> enc_RoadGeometryAndAttributes(Data);
encode_disp(Type, _Data) -> exit({error,{asn1,{undefined_type,Type}}}).

decode_disp('RoadGeometryAndAttributes', Data) -> dec_RoadGeometryAndAttributes(Data);
...
enc_RoadGeometryAndAttributes(Val) ->
[].


dec_RoadGeometryAndAttributes(Bytes) ->
begin
ignore,
{'NULL',Bytes}
end.

typeinfo('RoadGeometryAndAttributes') ->
  'NULL';
typeinfo(Type) ->
