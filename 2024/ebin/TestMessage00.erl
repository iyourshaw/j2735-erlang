...
%% Purpose: Encoding and decoding of the types in TestMessage00.

-module('TestMessage00').
-moduledoc false.
-compile(nowarn_unused_vars).
-dialyzer(no_improper_lists).
-dialyzer(no_match).
-include("TestMessage00.hrl").
-asn1_info([{vsn,'5.3.1'},
            {module,'TestMessage00'},
...
try encode_jer('TestMessage00', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('TestMessage00', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
typeinfo('TestMessage00') ->
  {sequence,'TestMessage00',2,[{<<104,101,97,100,101,114>>,{typeinfo,{'Common','Header'}},'OPTIONAL'},{<<114,101,103,105,111,110,97,108>>,{sequence,'TestMessage00_regional',2,[{<<114,101,103,105,111,110,73,100>>,{'INTEGER',{0,255}},mandatory},{<<114,101,103,69,120,116,86,97,108,117,101>>,{'ObjClassFieldType',{regionId,1},{componentrelation,{objectset,134,{'Externaltypereference',undefined,'TestMessage00',internal_object_set_argument_1}},[{outermost,[{'Externalvaluereference',134,'Common',regionId}]}]}},mandatory}]},'OPTIONAL'}]};
typeinfo(Type) ->
