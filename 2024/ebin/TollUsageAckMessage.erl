...
%% Purpose: Encoding and decoding of the types in TollUsageAckMessage.
...
-dialyzer(no_match).
-include("TollUsageAckMessage.hrl").
-asn1_info([{vsn,'5.3.1'},
            {module,'TollUsageAckMessage'},
...
-export([typeinfo/1]).
-export([
enc_TollUsageAckMessage/1,
enc_TumAck/1,
enc_TumAckHash/1
]).

-export([
dec_TollUsageAckMessage/1,
dec_TumAck/1,
dec_TumAckHash/1
]).
...
try encode_jer('TollUsageAckMessage', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('TollUsageAckMessage', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
encode_disp('TollUsageAckMessage', Data) -> enc_TollUsageAckMessage(Data);
encode_disp('TumAck', Data) -> enc_TumAck(Data);
encode_disp('TumAckHash', Data) -> enc_TumAckHash(Data);
encode_disp(Type, _Data) -> exit({error,{asn1,{undefined_type,Type}}}).

decode_disp('TollUsageAckMessage', Data) -> dec_TollUsageAckMessage(Data);
decode_disp('TumAck', Data) -> dec_TumAck(Data);
decode_disp('TumAckHash', Data) -> dec_TumAckHash(Data);
...
enc_TollUsageAckMessage(Val) ->
[<<0:1>>,
begin
%% attribute tumAck(1) with type TumAck
Enc2@element = element(2, Val),
enc_TumAck(Enc2@element)
end|begin
%% attribute ackMaxAge(2) with type INTEGER
Enc3@element = element(3, Val),
Enc3@element@sub = Enc3@element - 100,
if 0 =< Enc3@element@sub, Enc3@element@sub < 9901 ->
<<0:1,Enc3@element@sub:14>>;
true ->
...
%% attribute tumAck(1) with type TumAck
{Term1,Bytes2} = dec_TumAck(Bytes1),

%% attribute ackMaxAge(2) with type INTEGER
{Term2,Bytes3} = begin
{V2@V0,V2@Buf1} = case Bytes2 of
<<0:1,V2@V3:14,V2@Buf4/bitstring>> ->
V2@Add5 = V2@V3 + 100,
{V2@Add5,V2@Buf4};
...
Res1 = {'TollUsageAckMessage',Term1,Term2},
{Res1,Bytes5}.
...
[<<Enc1@len@sub:5>>|[enc_TumAckHash(Comp) || Comp <- Val]]
end.
...
begin
%% attribute signedTumHash(1) with type OCTET STRING
...
1 ->
begin
<<V3@V0:32/binary-unit:8,V3@Buf1/bitstring>> = Bytes2,
...
Res1 = {'TumAckHash',Term1,Term2,Term3},
{Res1,Bytes7}.

typeinfo('TollUsageAckMessage') ->
  {sequence,'TollUsageAckMessage',2,[{<<116,117,109,65,99,107>>,{typeinfo,{'TollUsageAckMessage','TumAck'}},mandatory},{<<97,99,107,77,97,120,65,103,101>>,'INTEGER',mandatory}]};
typeinfo('TumAck') ->
  {sof,{typeinfo,{'TollUsageAckMessage','TumAckHash'}}};
typeinfo('TumAckHash') ->
  {sequence,'TumAckHash',3,[{<<115,105,103,110,101,100,84,117,109,72,97,115,104>>,octet_string,'OPTIONAL'},{<<116,101,109,112,73,100>>,octet_string,mandatory},{<<116,117,109,83,101,113,117,101,110,99,101,78,117,109>>,{'INTEGER',{0,127}},mandatory}]};
typeinfo(Type) ->
...
{Term,Remain} = dec_TumAckHash(Bytes),
