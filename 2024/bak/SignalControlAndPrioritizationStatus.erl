...
%% Purpose: Encoding and decoding of the types in SignalControlAndPrioritizationStatus.
...
-export([typeinfo/1]).
-export([
enc_SignalControlAndPrioritizationStatus/1
]).

-export([
dec_SignalControlAndPrioritizationStatus/1
]).
...
try encode_jer('SignalControlAndPrioritizationStatus', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('SignalControlAndPrioritizationStatus', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
encode_disp('SignalControlAndPrioritizationStatus', Data) -> enc_SignalControlAndPrioritizationStatus(Data);
encode_disp(Type, _Data) -> exit({error,{asn1,{undefined_type,Type}}}).

decode_disp('SignalControlAndPrioritizationStatus', Data) -> dec_SignalControlAndPrioritizationStatus(Data);
...
enc_SignalControlAndPrioritizationStatus(Val) ->
[].


dec_SignalControlAndPrioritizationStatus(Bytes) ->
begin
ignore,
{'NULL',Bytes}
end.

typeinfo('SignalControlAndPrioritizationStatus') ->
  'NULL';
typeinfo(Type) ->
