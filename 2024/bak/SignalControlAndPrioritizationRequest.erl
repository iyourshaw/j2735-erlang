...
%% Purpose: Encoding and decoding of the types in SignalControlAndPrioritizationRequest.
...
-export([typeinfo/1]).
-export([
enc_SignalControlAndPrioritizationRequest/1
]).

-export([
dec_SignalControlAndPrioritizationRequest/1
]).
...
try encode_jer('SignalControlAndPrioritizationRequest', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('SignalControlAndPrioritizationRequest', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
encode_disp('SignalControlAndPrioritizationRequest', Data) -> enc_SignalControlAndPrioritizationRequest(Data);
encode_disp(Type, _Data) -> exit({error,{asn1,{undefined_type,Type}}}).

decode_disp('SignalControlAndPrioritizationRequest', Data) -> dec_SignalControlAndPrioritizationRequest(Data);
...
enc_SignalControlAndPrioritizationRequest(Val) ->
[].


dec_SignalControlAndPrioritizationRequest(Bytes) ->
begin
ignore,
{'NULL',Bytes}
end.

typeinfo('SignalControlAndPrioritizationRequest') ->
  'NULL';
typeinfo(Type) ->
