...
try encode_jer('NTCIP', Type, Data) of
  Bytes ->
    {ok,Bytes}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
...
try
   Result = decode_jer('NTCIP', Type, json:decode(Data)),
   {ok,Result}
  catch
    Class:Exception:Stk when Class =:= error; Class =:= exit ->
      case Exception of
...
typeinfo('EssMobileFriction') ->
  {'INTEGER',{0,101}};
typeinfo('EssPrecipRate') ->
  {'INTEGER',{0,65535}};
typeinfo('EssPrecipSituation') ->
  {'ENUMERATED',#{other => 1,unknown => 2,noPrecipitation => 3,unidentifiedSlight => 4,unidentifiedModerate => 5,unidentifiedHeavy => 6,snowSlight => 7,snowModerate => 8,snowHeavy => 9,rainSlight => 10,rainModerate => 11,rainHeavy => 12,frozenPrecipitationSlight => 13,frozenPrecipitationModerate => 14,frozenPrecipitationHeavy => 15}};
typeinfo('EssPrecipYesNo') ->
  {'ENUMERATED',#{error => 3,precip => 1,noPrecip => 2}};
typeinfo('EssSolarRadiation') ->
  {'INTEGER',{0,65535}};
typeinfo(Type) ->
